...
export const ScanView: React.FC<ScanViewProps> = ({ /* geminiApiKeyConfigured = false, */ onAddGames }) => {
  ...
  const [selectedAiService, setSelectedAiService] = useState<string>('gemini'); // Default to gemini

  useEffect(() => {
    const storedAiService = localStorage.getItem(AI_STORAGE_KEY);
    if (storedAiService) {
      setSelectedAiService(storedAiService);
    }
  ...
  const handleFetchDetailsWithAI = async () => {
    ...
    if (selectedAiService === 'mock') {
      const mockEnrichedGames: Record<string, Partial<ScannedFile>> = {};
      selectedFiles.forEach(file => {
        mockEnrichedGames[file.id] = {
          title: `Mocked: ${file.name.substring(0, file.name.lastIndexOf('.')) || file.name}`,
          description: 'This is a mock description generated by the Mock AI service.',
          genre: 'Mock Genre',
          releaseDate: '2024',
        };
      });
      ...
      setScannedFiles(prevFiles =>
        prevFiles.map(file => {
          if (mockEnrichedGames[file.id]) {
            // Ensure it remains selected, spread existing file properties first
            return { ...file, ...mockEnrichedGames[file.id], selected: true };
          }
          return file;
        })
      );
      setIsLoadingAi(false);
      alert("Mock AI details generated successfully!");
      return; // Skip real API call
    }

    // Real AI service call (Gemini or GitHub, assuming GitHub uses the same endpoint for now)
    const batchSize = 20;
    ...
    for (let i = 0; i < selectedFiles.length; i += batchSize) {
      ...
      console.log(`Sending game list to ${selectedAiService.toUpperCase()} for batch:`, gameListForApi.map(g => g.title));

      try {
        // TODO: If 'github' AI needs a different endpoint, add conditional logic here
        const response = await fetch('/api/gemini/enrich-gamelist', {
          ...
          body: JSON.stringify({
            ...
            // Optionally pass selectedAiService if backend needs to differentiate
        ...
        if (!response.ok) {
          ...
          console.error(`Error from ${selectedAiService.toUpperCase()} API proxy:`, errorData);
          alert(`Error fetching details from ${selectedAiService.toUpperCase()}: ${errorData.message || errorData.error || response.statusText}`);
          continue;
        }

        const result = await response.json();
        console.log(`${selectedAiService.toUpperCase()} Enrich Result:`, result);
        ...
        if (result.enriched_games && Array.isArray(result.enriched_games)) {
          result.enriched_games.forEach((enrichedGame: any, index: number) => {
            ...
            if (originalFile && enrichedGame.title && enrichedGame.description) {
              ...
              console.warn(`Missing title or description for ${originalFile.name} in ${selectedAiService.toUpperCase()} response:`, enrichedGame);
            }
          });
        } else {
           console.warn(`${selectedAiService.toUpperCase()} response did not contain an enriched_games array:`, result);
        }

      } catch (error) {
        console.error(`Network error or other issue calling ${selectedAiService.toUpperCase()} proxy:`, error);
        ...
        <div className="flex-grow bg-neutral-800 p-6 rounded-lg shadow-xl flex flex-col min-h-0">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex space-x-2">
              { (selectedAiService === 'mock' ||
                (selectedAiService === 'gemini' && isGeminiConfigured) ||
                (selectedAiService === 'github' && isGeminiConfigured) /* Assuming GitHub uses same config flag for now */
                ) && (
                <Button
                  ...
                  variant="secondary"
                >
                  {isLoadingAi ? `Fetching Details via ${selectedAiService.toUpperCase()}...` : `Fetch Details with ${selectedAiService.toUpperCase()}`}
                </Button>
              )}
              <Button
